...
from lib.utilities import reachable_services, registered_submodules
...
class RepoOperation(BaseOperation):
    ...
    def _target(self, mandatory):
        ...
        (models|spaces|datasets); the service must be reachable (registered
        AND with credentials in .env) — and only huggingface hosts git
        repos loadable as submodules, so anything else errors here."""
