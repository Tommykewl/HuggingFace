"""`unload <entity> <namespace> <name> [-f]` — unload a loaded submodule.
...
class UnloadOperation(RepoOperation):
    """`unload <entity> <namespace> <name> [-f]` — deinit the submodule
    (folder emptied, stays registered); -f removes it entirely (worktree,
    gitlink, .gitmodules entry, .git/modules cache)."""

    _helptext = [
        ("unload models <namespace> <model_name> [-f]",
         ...
         "submodule, refuses if it has uncommitted changes, then `git\n"
         "submodule deinit`s it — the folder is emptied locally but stays\n"
         "registered, so a later `load models` restores it.\n"
         ...
         "Example: unload models smallTech rtdetr-sportsmot -f"),
        ("unload spaces <namespace> <space_name> [-f]",
         "Unload the local submodule at hf/<namespace>/spaces/<space_name>.\n"
         "Same checks and behaviour as `unload models`."),
        ("unload datasets <namespace> <dataset_name> [-f]",
    ...
    def _run(self, mandatory, optional, vargs):
        ...
        if not force:
            ...
            print(f"Unloaded: {rel} (still registered — `load {entity} {ns} "
                  f"{name}` restores it)")
            return
